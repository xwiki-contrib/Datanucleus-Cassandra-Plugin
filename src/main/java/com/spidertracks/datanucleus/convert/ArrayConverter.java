 ...
 * A null entry will corrispond to a single 0xFE byte.
 *
 * @version $Id$
 * @since 1.1.1-0.7.0
 */
public class ArrayConverter implements ByteConverter
{
    ...
    public Object getObject(final ByteBuffer buffer, final ByteConverterContext context)
    {
        ...
        for (;;) {
            ...
            if (length < 0) {
                // -1 length indicates a null entry.
                out.add(null);
                next++;
                continue;
            }
                                 ...
                                 final ByteConverterContext context)
    {
        ...
        for (int i = 0; i < values.length; i++) {
            if (values[i] == null) {
                out = ConverterUtils.check(out, 1);
                writeLength(-1, out);
                continue;
            }
     ...
     * If length is less than 0 (used to indicate a null entry), a single 0xFE byte is stored.
     *
     ...
     * If the length input to writeLength was less than 0, -1 is returned.
     *
